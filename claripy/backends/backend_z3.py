...
from .KNN_Predictor import Predictor
import time
...
class BackendZ3(Backend):
    ...
    predictor = Predictor(query_record.filename, timeout_threshold)
    KNN_count = 0
    bonus_time = 0
    ...
    def check(solver):
        ...
        if result == z3.unknown:
            BackendZ3.KNN_count += 1
            query_smt2 = solver.to_smt2()
            print(BackendZ3.query_record.filename, "predictor is triggered")
            pred_result = BackendZ3.predictor.predict(query_smt2)
            if pred_result == 1:
                result = z3.unknown
                start_time = time.time()
                result = solver.check()
                end_time = time.time()
                time_delta = end_time - start_time
                BackendZ3.bonus_time += time_delta
                print(BackendZ3.query_record.filename, "predicted solving time exceed threshold, skip the query")
            else:
                solver.set('timeout', BackendZ3.timeout_threshold * 1000)
                start_time = time.time()
                result = solver.check()
                end_time = time.time()
                if result == z3.unknown:
                    truth = 1
                    print(BackendZ3.query_record.filename, "prediction error, update the model")
                else:
                    truth = 0
                    print(BackendZ3.query_record.filename, "prediction correct, update the model")
                BackendZ3.predictor.increment_KNN_data(truth)
                time_delta = end_time - start_time
            BackendZ3.query_record.record_predictor_data(query_smt2, pred_result, time_delta)
        BackendZ3.query_record.update(query_smt2, time_delta)
        return result
