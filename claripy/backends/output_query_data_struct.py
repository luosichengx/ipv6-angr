...
class query_data:
    ...
    def record_predictor_data(self, query, pred_result, time_delta):
        output = {"filename": self.filename, "script": query, "predict_time": float(pred_result), "time": time_delta,
                  "stamp": str(time.localtime())}
        try:
            # # record query
            with open(self.query_output_dir + self.filename + str(self.query_index) + "p", "w") as f:
                json.dump(output, f, indent=4)
        except (IOError, KeyError, IndexError, TypeError):
            pass