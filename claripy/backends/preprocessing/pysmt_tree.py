...
class pysmt_query_tree(query_tree):
    ...
    def parse_klee_smt(self, asserts):
        ...
        for assertion in script.commands:
            ...
            # else:
            #     last_feature_sum = self.count_feature(asserts[-1])
            #     last_feature = self.count_feature(fnode.to_smtlib())
            #     last_feature_sum = [last_feature_sum[i] - last_feature[i] for i in range(len(last_feature_sum))]
            #     feature_list.append(last_feature_sum)
