...
if __name__ == '__main__':
    ...
    if args.dir is not None:
        ...
        # exlude_list = ["linuxrc"]
        # exlude_list.extend(map(lambda x: x.split(".")[0], os.listdir("/home/lsc/busybox_data1/log/log")))
        # exlude_list.extend(map(lambda x: x.split(".")[0], os.listdir("/home/lsc/busybox_data2/log/log")))
        for root, dirs, files in os.walk(dirpath):
            if filename_list is not None:
                files = filename_list.split(",")
            for file in files:
                # if file in exlude_list:
                #     continue
