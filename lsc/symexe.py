...
def run_symexe(path, argv_size=8, withtime=True):
    ...
    if cf.getboolean("Angr", "output"):
        ...
        try:
            claripy._backends_module.backend_z3.Predictor.filename = path.split("/")[-1]
        except:
            pass
    else:
        ...
        if withtime:
            ...
            def my_step_func(lpg):
                ...
                if lpg.deadended:
                    for pg in lpg.deadended:
                        ...
                        logger.info("result")
                        logger.info(pg.solver.eval(pg.solver.temporal_tracked_variables[('api', '?', 'getopt_long', 1)], cast_to=bytes))
                        # print(pg.solver.eval(pg.solver.temporal_tracked_variables[('api', '?', 'pipe', 1)], cast_to=bytes))
            ...
            for _ in (itertools.count()):
                if not pg.complete() and pg._stashes['active']:
                    ...
                    try:
                        tel_all = tel + claripy._backends_module.backend_z3.bonus_time
                    except (AttributeError):
                        tel_all = tel
                    if time_delta > tel_all:
    ...
    try:
        if cfg != None:
            ...
            if main_obj != None:
                ...
                own_addr = [p.entry]
            own_node = [cfg.get_any_node(own_addr[0])]
            import networkx
